...
df = pd.read_csv(r"C:\Users\hi2kh\OneDrive\Documents\GitHub\Machine-Learning\shape_data\shape_data.csv",
                 names=[str(i) for i in range(98*98 + 1)])
...
x_train = np.reshape(x_train, (-1, 98, 98, 1)) / 255
x_test = np.reshape(x_test, (-1, 98, 98, 1)) / 255
...
y_train = np.reshape(y_train, (-1, 13))  # 12 different shapes, one hot encoded
y_test = np.reshape(y_test, (-1, 13))

#inputs = tf.keras.Input(shape=(98, 98, 1))
...
inputs = tf.keras.layers.Input(shape=(98, 98, 3))
...
model.fit(x=x_train, y=y_train, batch_size=64, epochs=10, validation_data=(x_test, y_test))#, callbacks=[callback])
